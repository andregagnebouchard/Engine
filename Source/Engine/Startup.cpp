...
namespace Engine
{
	bool Startup::CheckStorage(const int64 diskSpaceNeeded)
	{
		...
		if (diskfree.avail_clusters < neededClusters)
		{
			...
			Logger::Log("CheckStorage Failure: Not enough physical storage", Logger::Level::Error);
			return false;
		}
		return true;
	}
	...
	bool Startup::CheckMemory(const uint64 physicalRAMNeeded, const uint64 virtualRAMNeeded)
	{
		...
		if (status.ullTotalPhys < physicalRAMNeeded)
		{
			...
			Logger::Log("CheckMemory Failure: Not enough physical memory.", Logger::Level::Error);
			return false;
		}
		...
		if (status.ullAvailVirtual < virtualRAMNeeded)
		{
			...
			Logger::Log("CheckMemory Failure: Not enough virtual memory.", Logger::Level::Error);
			return false;
		}
		...
		else
		{
			...
			Logger::Log("CheckMemory Failure: Not enough contiguous available memory.", Logger::Level::Error);
			return false;
		}
		return true;
	}
